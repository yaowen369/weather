...
public class LunarCalendar {
        ...
        //yaowen 添加的方法。利用该方法的solarToLunar（），如果该方法返回的 leap3 = 1,
         //则使返回的月份自动加1
         //这样返回的的数组只需要使用三位   year month day
        public static final int[] ywSolarToLunar(int year, int month, int monthDay){
        	int[] result = solarToLunar(year, month, monthDay);
        	int[] retResult = new int[3];
        	retResult[0] = result[0];
        	retResult[1] = result[1] + result[3];
        	retResult[2] = result[2];
        	
        	MyLog.v(TAG, "result = " + result[0] + " " + result[1] + " " + result[2] + " " + result[3]
        				+ "\n retResult = " + retResult[0] + " " + retResult[1] + " " + retResult[2]);
